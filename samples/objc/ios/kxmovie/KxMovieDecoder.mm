...
static NSData * copyFrameData(UInt8 *src, int linesize, int width, int height)
{
    ...
    Byte *dst = reinterpret_cast<Byte*>(md.mutableBytes);
...
/*  int size;
  ...
  size = FFMIN(buf_size, bd->size);
  printf("opaque:%p ptr:%p size:%i\n", opaque, bd->ptr, size);
  
  memcpy(buf, bd->ptr, size);
  printf("buf is %s and bd->ptr is %s", buf, bd->ptr);
  bd->ptr  += size;
  bd->size -= size;
  ...
  const unsigned char *bytePtr =
    reinterpret_cast<const unsigned char *>([buffer bytes]);
...
- (KxAudioFrame *) handleAudioFrame
{
    ...
    if (_swrContext) {
        ...
        Byte *outbuf[2] = {reinterpret_cast<Byte*>(_swrBuffer), 0 };
    ...
    vDSP_vflt16((SInt16 *)audioData, 1,
                reinterpret_cast<float*>(data.mutableBytes), 1, numElements);
    vDSP_vsmul(reinterpret_cast<const float*>(data.mutableBytes), 1, &scale,
               reinterpret_cast<float*>(data.mutableBytes), 1,
               numElements);
